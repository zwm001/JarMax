...
public class WebBasic {
    public static JSONObject parse(LinkedHashMap<String, String> jx, String url) {
        try {
            SpiderDebug.log("Load Web Parse Basic...");
            if (jx.size() > 0) {
                ...
                for (String jxName : jxNames) {
                    ...
                    try {
                        ...
                        String web = OkHttpUtil.string(realUrl + url, reqHeaders);
                        JSONObject taskResult = Misc.jsonParse(url, web);
                        if (taskResult == null)
                            continue;
        ...
        if (url.contains("cat_ext")) {
            try {
                ...
                if (jsonObject.has("header")) {
                    JSONObject headerWeb = jsonObject.optJSONObject("header");
                    Iterator<String> keys = headerWeb.keys();
                    while (keys.hasNext()) {
                        String key = keys.next();
                        reqHeaders.put(key, headerWeb.optString(key, ""));
                    }
                }
