            ...
            for (int i = 0; i < allScript.size(); i++) {
                ...
                if (scContent.startsWith("var player_")) { // 取直链
                    ...
                    if (playerConfig.has(player.getString("from"))) {
                        ...
                        if (player.getString("from").contains("LINE") || player.getString("from").contains("ali")) {
 ...
 //   }
    ...
    public String searchContent(String key, boolean quick) {
        try {
            ...
            JSONObject result = new JSONObject();
            JSONArray videos = new JSONArray();
            Elements list = doc.select(".col-md-6.col-sm-4.col-xs-3");
            for (int i = 0; i < list.size(); i++) {
                Element vod = list.get(i);
                String title = vod.selectFirst(".stui-vodlist__box a").attr("title");
                String cover = doReplaceRegex(m, vod.selectFirst(".stui-vodlist__box a").attr("data-original"));
                Matcher matcher = regexVid.matcher(vod.selectFirst(".stui-vodlist__box a").attr("href"));
